...
import { getAllResults, getAllResultsNotValuated } from "../../helper/helperResults"; // Import the function to fetch results
...
export default function ListNotValidateAnalyse() {
  ...
  const setSelectedResultsValidate = useStore(
    (state) => state.setSelectedResultsValidate
  );
  ...
  const handleFilteredResults = (event) => {
    ...
    const filteredRows = results.filter((row) =>
      `${row.patientName} ${row.dateCreation}`.toLowerCase().includes(input)
    );
  ...
  useEffect(() => {
    getAllResultsNotValuated() // Call the API function to fetch results
      .then((results) => {
        setResults(results);
  ...
  const handleButtonClick = (row) => {
    setSelectedResultsValidate(row);
    navigate("/validateResult", {
      state: { activePage: "/validatePrescriptionResults", activeDashboard: test },
    });
  };
