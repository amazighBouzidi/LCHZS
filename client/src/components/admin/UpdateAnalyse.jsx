...
import { updateAnalyse } from '../../helper/helperAnalyse';



// Define the AddAnalyse component
export default function UpdateAnalyse({ open, onClose, row, updateAnalysisInList }) {
    const initialValues = {
        nom: row.nom,
        code: row.code,
        prix: row.prix,
    ...
    const handleUpdate = async (values) => {
        updateAnalyse(row._id, values)
        .then(({msg, newUpdatedAnalyse})=>{
            console.log(msg)
            updateAnalysisInList(newUpdatedAnalyse); // Notify parent component
            onClose(); // Close the modal
        })
        .catch((err)=>console.error(`Error Updating the Analyse : ${err}`))
    }
